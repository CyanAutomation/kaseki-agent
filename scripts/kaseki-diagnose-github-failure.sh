...
json_value() {
  local file="$1"
  local key="$2"
  grep "\"$key\"" "$file" | head -1 | sed -E 's/.*"'"$key"'"[[:space:]]*:[[:space:]]*//; s/[[:space:]]*,[[:space:]]*$//; s/^"//; s/"$//'
}
...
GITHUB_OPERATION_PHASE="$(json_value "$RESULTS_DIR/metadata.json" "github_operation_phase")"
...
printf -- '- GitHub operation phase: %s\n' "${GITHUB_OPERATION_PHASE:-unknown}"
...
if [ "$GITHUB_OPERATION_PHASE" = "token_generation" ] || [ "$GITHUB_API_ERROR_TYPE" = "github_app_token_error" ]; then
  printf '**GitHub App token generation failed (exit code: %s)**\n\n' "$GITHUB_PUSH_EXIT"
  if [ -n "$GITHUB_API_ERROR_MESSAGE" ]; then
    printf 'Token error message: %s\n' "$GITHUB_API_ERROR_MESSAGE"
  fi
  if [ -n "$GITHUB_API_HTTP_STATUS" ]; then
    printf 'HTTP status: %s\n' "$GITHUB_API_HTTP_STATUS"
  fi
  printf '\nGitHub App token generation failed. Check git-push.log for details:\n'
  if [ -f "$RESULTS_DIR/git-push.log" ]; then
    printf '\n```\n'
    tail -20 "$RESULTS_DIR/git-push.log"
    printf '```\n'
  fi
elif [ "$GITHUB_PUSH_EXIT" -ne 0 ] && [ "$GITHUB_PR_EXIT" -eq 0 ]; then
