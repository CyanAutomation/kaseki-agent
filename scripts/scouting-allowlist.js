...
function summarize(errors) {
  ...
  const counts = [];
  if (critical)
    counts.push(`${critical} critical`);
  if (warning)
    counts.push(`${warning} warning`);
...
function validateSuggestedAllowlist(suggestedAllowlist, errors) {
  ...
  else if (!sal.agent_patterns.every((p) => typeof p === 'string')) {
    errors.push({
      field: 'suggested_allowlist.agent_patterns',
      expected: 'array of strings',
  ...
  else if (!sal.validation_patterns.every((p) => typeof p === 'string')) {
    errors.push({
      field: 'suggested_allowlist.validation_patterns',
      expected: 'array of strings',
      actual: 'array with non-strings',
...
function writeValidationArtifacts(result, options) {
  ...
  const jsonlLog = options.jsonlLog;
  if (result.status === 'ok')
    return;
  if (jsonlLog) {
...
function deriveAllowlistFromScoutingArtifact(artifact) {
  ...
  return {
    agentAllowlist: art &&
            art.suggested_allowlist?.agent_patterns &&
            Array.isArray(art.suggested_allowlist.agent_patterns)
      ? art.suggested_allowlist.agent_patterns.join(' ')
      : '',
    validationAllowlist: art &&
            art.suggested_allowlist?.validation_patterns &&
            Array.isArray(art.suggested_allowlist.validation_patterns)
      ? art.suggested_allowlist.validation_patterns.join(' ')
      : '',
  };
}
...
function mergeAllowlists(scoutingPatterns = '', userPatterns = '') {
  if (scoutingPatterns && userPatterns)
...
//# sourceMappingURL=scouting-allowlist.js.map