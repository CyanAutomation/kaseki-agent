...
function generateReport(resultsDir) {
  ...
  lines.push('');
  if (findings.length > 0) {
    lines.push('- Review the findings above');
    lines.push('- Consider next steps based on discovered issues');
  } else {
  ...
  lines.push('---');
  lines.push('');
  lines.push('*Generated by Kaseki inspect mode*');
...
if (!resultsDir) {
  console.error('Usage: node generate-inspect-report.js <results-dir>');
  process.exit(1);
}

try {
  const report = generateReport(resultsDir);
  const outputPath = path.join(resultsDir, 'inspect-report.md');
  fs.writeFileSync(outputPath, report, 'utf8');
  console.log(`Generated: ${outputPath}`);
} catch (error) {
  console.error('Error generating inspect report:', error.message);
  process.exit(1);
}
