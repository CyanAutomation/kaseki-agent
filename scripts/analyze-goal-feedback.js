...
function analyzeGoalFeedback(entries) {
    ...
    for (const entry of goalCheckEntries) {
        ...
        if (score >= buckets.high.min)
            buckets.high.entries.push(entry);
        else if (score >= buckets.medium.min)
            buckets.medium.entries.push(entry);
        else
            buckets.low.entries.push(entry);
    }
    ...
    for (const [key, bucket] of Object.entries(buckets)) {
        ...
        stats[key] = {
            ...
            avg_quality_score: ((bucket.entries.reduce((sum, e) => sum + (e.goal_quality?.score || 0), 0) /
                ...
                .toFixed(1)),
            avg_completion_attempts: ((bucket.entries.reduce((sum, e) => sum + (e.outcomes?.coding_attempts || 1), 0) /
                bucket.entries.length)
                .toFixed(1)),
        };
    }
...
function analyzeCorrelations(entries) {
    ...
    const highQualitySuccessRate = entries
        ...
        Math.max(1, entries.filter((e) => (e.goal_quality?.score || 0) >= 85).length);
    ...
    const highConfidenceCorrect = entries
        ...
        .reduce((sum, e) => sum +
        (e.goal_check_verdict?.met ===
            e.correlation?.success ? 1 : 0), 0) / Math.max(1, entries.filter((e) => e.goal_check_verdict?.confidence === 'high').length);
    ...
    const avgEvidenceCount = entries.reduce((sum, e) => sum + (e.goal_check_verdict?.evidenceCount || 0), 0) /
        entries.length;
    const avgMissingCount = entries.reduce((sum, e) => sum + (e.goal_check_verdict?.missingCount || 0), 0) /
        entries.length;
...
function analyzeSmartDimensions(entries) {
    ...
    for (const entry of entries) {
        ...
        for (const criterion of smartCriteria) {
            ...
            const score = (crit.smart_score || 'unknown');
...
//# sourceMappingURL=analyze-goal-feedback.js.map