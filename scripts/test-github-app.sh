...
echo ""
echo "Test 3: Checking dist/github-app-token.js..."
if [ -x "$ROOT_DIR/dist/github-app-token.js" ]; then
  echo "✓ dist/github-app-token.js exists and is executable"
else
  echo "✗ dist/github-app-token.js is NOT executable"
  exit 1
fi

# Test 4: Verify github-app-token.js has required functions
echo ""
echo "Test 4: Checking dist/github-app-token.js content..."
if grep -q "generateJWT" "$ROOT_DIR/dist/github-app-token.js" && \
   grep -q "getInstallationId" "$ROOT_DIR/dist/github-app-token.js" && \
   grep -q "getAccessToken" "$ROOT_DIR/dist/github-app-token.js"; then
  echo "✓ dist/github-app-token.js contains required functions"
else
  echo "✗ dist/github-app-token.js missing required functions"
  exit 1
fi
...
echo ""
echo "Test 9: Testing dist/github-app-token.js argument validation..."
output=$(node "$ROOT_DIR/dist/github-app-token.js" 2>&1 || true)
if echo "$output" | grep -q "Usage:"; then
  echo "✓ dist/github-app-token.js shows usage message"
else
  echo "✗ dist/github-app-token.js usage message not shown"
  exit 1
fi
...
echo "  ✓ dist/github-app-token.js helper script is complete"
echo "  ✓ Dockerfile includes dist/github-app-token.js"
