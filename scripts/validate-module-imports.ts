...
function resolveDependencies(
  ...
  results: Map<string, DependencyInfo> = new Map(),
  ...
  try {
    ...
    results.set(normalizedPath, { exists: true, imports: [], error: (error as Error).message });
    return results;
  }
...
function main(): void {
  ...
  const criticalBinaries: BinaryConfig[] = [
  ...
  for (const binary of criticalBinaries) {
    ...
    for (const [depPath, depInfo] of dependencies) {
      if (depPath === sourceFile) continue; // Skip the source file itself
