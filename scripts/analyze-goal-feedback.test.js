...
const { readFeedbackFile, analyzeGoalFeedback, analyzeCorrelations, analyzeSmartDimensions, generateRecommendations, } = script;
...
{"phase":"goal_check","goal_quality":{"score":85}}`;
  ...
  describe('analyzeGoalFeedback', () => {
    ...
    test('should calculate average quality score per bucket', () => {
      ...
      const result = analyzeGoalFeedback(entries);
      const highAvg = parseFloat(result.quality_buckets?.high?.avg_quality_score || '0');
  ...
  describe('analyzeCorrelations', () => {
    test('should detect high-quality goal success signal', () => {
      ...
      const signalNote = result.find((n) => n.includes('Strong signal'));
    ...
    test('should measure evaluator calibration accuracy', () => {
      ...
      const result = analyzeCorrelations(entries);
      const calibrationNote = result.find((n) => n.includes('calibration'));
    ...
    test('should compute average evidence and missing item counts', () => {
      ...
      const result = analyzeCorrelations(entries);
      const effortNote = result.find((n) => n.includes('Evaluator effort'));
  ...
  describe('generateRecommendations', () => {
    test('should generate high-priority goal quality recommendation when strong signal', () => {
      ...
      const qualityRec = result.find((r) => r.area === 'goal_quality');
    ...
    test('should generate high-priority evaluator quality recommendation', () => {
      ...
      const smartAnalysis = { distribution: {} };
      const result = generateRecommendations(stats, correlationNotes, smartAnalysis);
      const evaluatorRec = result.find((r) => r.area === 'evaluator_quality');
    ...
    test('should generate medium-priority SMART criteria recommendation when >20% low', () => {
      ...
      const result = generateRecommendations(stats, correlationNotes, smartAnalysis);
      const smartRec = result.find((r) => r.area === 'smart_criteria');
    ...
    test('should not generate goal quality recommendation when high/low counts missing', () => {
      ...
      const smartAnalysis = { distribution: {} };
      const result = generateRecommendations(stats, correlationNotes, smartAnalysis);
      expect(result.find((r) => r.area === 'goal_quality')).toBeUndefined();
    });
    ...
    test('should not generate SMART criteria recommendation when low < 20%', () => {
      ...
      const result = generateRecommendations(stats, correlationNotes, smartAnalysis);
      expect(result.find((r) => r.area === 'smart_criteria')).toBeUndefined();
    });
  });
  // ===== Integration Tests =====
  describe('analyzeGoalFeedback integration', () => {
    ...
    test('should read representative feedback, skip malformed records, and produce a stable analysis contract', () => {
      ...
      fs.writeFileSync(testFile, feedbackLines.map((entry) => (typeof entry === 'string' ? entry : JSON.stringify(entry))).join('\n'));
      try {
        ...
        expect(analysis.recommendations.map((rec) => `${rec.priority}:${rec.area}`)).toEqual([
          'high:goal_quality',
    ...
    test('should calculate correct success rates across quality buckets', () => {
      ...
      const result = analyzeGoalFeedback(entries);
      const highSuccessRate = parseFloat(result.quality_buckets?.high?.success_rate || '0');
      const lowSuccessRate = parseFloat(result.quality_buckets?.low?.success_rate || '0');
    ...
    test('should handle verdicts that sometimes match correlation', () => {
      ...
      const result = analyzeCorrelations(entries);
      const verdictNote = result.find((n) => n.includes('verdict_met_rate'));
    ...
    test('should accumulate attempt counts across buckets', () => {
      ...
      const result = analyzeGoalFeedback(entries);
      const highAttempts = parseFloat(result.quality_buckets?.high?.avg_completion_attempts || '0');
      const lowAttempts = parseFloat(result.quality_buckets?.low?.avg_completion_attempts || '0');
  ...
  describe('edge cases and error handling', () => {
    test('readFeedbackFile should handle large files gracefully', () => {
      ...
      expect(analysis.quality_buckets?.high).toEqual(expect.objectContaining({ count: 300, success_rate: '100.0', avg_quality_score: '89.3' }));
      expect(analysis.quality_buckets?.medium).toEqual(expect.objectContaining({ count: 300, success_rate: '100.0', avg_quality_score: '72.0' }));
      expect(analysis.quality_buckets?.low).toEqual(expect.objectContaining({ count: 400, success_rate: '0.0', avg_quality_score: '32.8' }));
      expect(analysis.smart_analysis).toEqual(expect.objectContaining({
...
//# sourceMappingURL=analyze-goal-feedback.test.js.map