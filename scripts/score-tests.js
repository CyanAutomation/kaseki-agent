...
function getAllTestFiles(rootDir) {
  const testFiles = [];
  function walk(dir) {
    try {
...
function extractTestName(content, fileExt) {
...
function scoreIntentClarity(content, fileName, fileExt) {
...
function scoreBehavioralRelevance(content) {
...
function scoreAssertionQuality(content, fileExt) {
...
function scoreIsolationRobustness(content) {
  const concerns = [];
  let score = 2;
...
function scoreCostCoverage(content) {
  const lines = content.split('\n').length;
...
function analyzeTest(filePath, content) {
  const fileExt = path.extname(filePath).slice(1);
  const language = fileExt === 'ts' ? 'typescript' : fileExt.match(/sh|bash/) ? 'shell' : 'other';
  ...
  if (content.match(/toBe\(/))
    assertionTypes.push('toBe');
  if (content.match(/toEqual\(/))
    assertionTypes.push('toEqual');
  if (content.match(/toContain\(/))
    assertionTypes.push('toContain');
  if (content.match(/toThrow\(/))
    assertionTypes.push('toThrow');
  if (content.match(/toMatchSnapshot/))
    assertionTypes.push('snapshot');
  const concerns = [];
  if (hasSnapshots && assertionTypes.length === 1)
    concerns.push('snapshot-only');
  if (lineCount > 150)
    concerns.push('overly-long');
  if (hasAsyncWaits && content.match(/setTimeout|delay.*\d{3,}/))
    concerns.push('timing-flake-risk');
  if (hasMocking && !hasSnapshots)
    concerns.push('heavy-internal-mocking');
  return {
...
main();
//# sourceMappingURL=score-tests.js.map