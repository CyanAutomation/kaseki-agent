...
function readJsonl(filePath: string): PiEvent[] {
  ...
  return content
    ...
    .filter(Boolean) as PiEvent[];
}

function readJson(filePath: string): PiSummary {
  const content = readFile(filePath);
  if (!content.trim()) return {};

  try {
    return JSON.parse(content) as PiSummary;
  } catch {
    return {};
  }
}
 ...
 */
function extractFindings(events: PiEvent[]): string[] {
  ...
  events.forEach((event) => {
    ...
    const sanitized = (event.content as string)
 ...
 */
function generateReport(resultsDir: string): string {
