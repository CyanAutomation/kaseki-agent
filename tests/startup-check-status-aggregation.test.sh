...
test_worker_preflight_blocks_when_a_scouting_template_is_missing() {
  ...
  local helper
  for helper in agent-prompt.sh evaluation-prompts.sh allowlist-helper.sh context-handoff.js restore-disallowed-changes.sh inspect-mode-defaults.sh dependency-cache-helpers.sh auto-lint-cleanup-classification.sh scouting-allowlist.js; do
    : > "$helper_root/$helper"
  done
