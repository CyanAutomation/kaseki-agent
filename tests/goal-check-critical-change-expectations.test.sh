...
EOF_VALIDATION_FILTER
  ...
  env PATH="$FAKE_BIN:$PATH" REPO_URL="$FAKE_REPO" GIT_REF=main TASK_PROMPT="$TASK_PROMPT_CASE" \
    ...
    KASEKI_PROVIDER=openrouter OPENROUTER_API_KEY=test GITHUB_APP_ENABLED=0 KASEKI_GIT_CACHE_MODE=off KASEKI_GOAL_CHECK_MAX_RETRIES=1 KASEKI_HASHLINE_EDITS=0 KASEKI_BASELINE_VALIDATION_ENABLED=0 KASEKI_TYPED_EVALUATION_ENABLED=0 \
