...
# This intentionally is not behavioral coverage. Run `npm run build`
# immediately before this check so failures indicate broken package output
...
assert.equal(typeof filterModule.filterPiEvents, 'function', 'pi-event-filter should export filterPiEvents');
assert.equal(typeof filterModule.shouldKeep, 'function', 'pi-event-filter should export shouldKeep');
assert.equal(typeof filterModule.sanitize, 'function', 'pi-event-filter should export sanitize');
