...
cp "$REPO_ROOT/scripts/validation-timeout-policy.sh" "$TMP_DIR/scripts/validation-timeout-policy.sh"
...
cp "$REPO_ROOT/scripts/lib/pi-json-capture.sh" "$TMP_DIR/scripts/lib/pi-json-capture.sh"
...
env PATH="$FAKE_BIN:$PATH" REPO_URL="$FAKE_REPO" GIT_REF=main TASK_PROMPT="make a required change" \
  KASEKI_PROVIDER=openrouter KASEKI_MODEL=z-ai/glm-4.5-air:free OPENROUTER_API_KEY=test \
