...
. "$ROOT_DIR/scripts/lib/maturity-score.sh"
...
workspace_dir="$TMP_DIR/workspace"; results_dir="$TMP_DIR/results"
mkdir -p "$workspace_dir/repo" "$results_dir"
export KASEKI_MATURITY_SCORE_EXECUTABLE="$fake_executable"
export KASEKI_TEST_INVOCATION_ENV_FILE="$TMP_DIR/invocation-env"
export KASEKI_TEST_INVOCATION_ARGS_FILE="$TMP_DIR/invocation-args"
...
node -e 'const a=require("node:assert/strict"),f=require("node:fs");a.deepEqual(JSON.parse(f.readFileSync(process.argv[1],"utf8")),{total_score:100,categories:{}})' "$results_dir/maturity-score.json"
missing_workspace_dir="$TMP_DIR/missing-workspace"; missing_results_dir="$TMP_DIR/missing-results"
