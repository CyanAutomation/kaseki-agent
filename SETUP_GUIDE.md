...
### Secret Storage
Store credentials in `~/.kaseki/secrets.json` with permissions `0600`. Point the selected provider's `*_API_KEY_FILE` variable to it; see [credential variables](docs/ENV_VARS.md#api-keys--credentials).

### Primary Provider: LLM Gateway

Kaseki defaults to an LLM Gateway. Select it explicitly with:
...
# Alternatively, select OpenRouter as the primary provider (not as a fallback):
# export KASEKI_PROVIDER=openrouter
# export OPENROUTER_API_KEY_FILE=~/.kaseki/openrouter-api-key
```

### Essential 8 Variables
Minimum 8 variables required; see [advanced configuration](docs/ADVANCED_CONFIG.md):
