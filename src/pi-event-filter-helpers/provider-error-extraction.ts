...
function firstPresentString(candidates: unknown[]): string | undefined {
...
export function extractStatusCode(message: any, nestedError: any): number | undefined {
  const statusCandidate = firstPresent([
    message?.statusCode,
    message?.status_code,
...
export function extractResponseId(message: any, event: PiEvent): string | undefined {
  return firstPresentString([message?.responseId, message?.response_id, (event as any).responseId]);
}
...
export function extractGatewayEventId(message: any, nestedError: any): string | undefined {
  return firstPresentString([
    message?.gatewayEventId,
    message?.gateway_event_id,
    nestedError?.eventId,
...
export function extractRetryAfter(message: any, nestedError: any): string | undefined {
  const retryAfterCandidate = firstPresent([
    message?.retryAfter,
    message?.retry_after,
