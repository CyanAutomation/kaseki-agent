...
describe('scorecard dimension tables and invariants', () => {
  ...
  test.each([
    ...
    const card = buildScorecard(collectEvidence(artifacts), NOW);
  ...
  test('missing critical evidence cannot increase confidence', () => {
    ScorecardContext.initialize(normalizeConfig({}));
    const complete = buildScorecard(collectEvidence(bundle()), NOW);
    const missing = buildScorecard(collectEvidence(bundle({ text: {} })), NOW);
  ...
  test('more retries or tokens cannot improve efficiency with other inputs equal', () => {
    ...
    expect(buildScorecard(costly, NOW).dimensions[2].normalized_score).toBeLessThanOrEqual(buildScorecard(base, NOW).dimensions[2].normalized_score);
  });
  ...
  test('failed required validation cannot receive top solution quality', () => {
    ...
    expect(buildScorecard(collectEvidence(failed), NOW).dimensions.find(d=>d.id==='validation_quality')?.normalized_score).toBe(0);
  });
  ...
  test('artifact ordering does not affect output', () => {
    ...
    expect(strip(buildScorecard(collectEvidence(artifacts),NOW))).toEqual(strip(buildScorecard(collectEvidence(reversed),NOW)));
  });
});
