...
/**
 * Classification Smoke Test Result: Evaluation endpoint contract check
 * Tests the structured decision endpoint with an evaluation-stage payload
 * CONSUMES inference tokens - only run when explicitly triggered
 */
export interface ClassificationSmokeTestResult {
  ...
  evaluationValidated?: boolean; // Classification payload was properly formed and responded
...
/**
 * @deprecated Retained for compatibility with the previous gateway smoke implementation
  ...
  if (!apiKeyResult.configured) {
    return {
      ...
      result: {
        ...
        responseTime: 0,
        timestamp,
        remediation: 'Set OPENROUTER_API_KEY or OPENROUTER_API_KEY_FILE before running the Evaluation stage check.',
      },
    };
  }
...
export async function testClassificationSmoke(requested: boolean = false): Promise<ClassificationSmokeTestResult> {
  ...
  if (!shouldRunClassificationSmoke(requested)) {
    return {
      status: 'skipped',
      detail: 'Evaluation stage check skipped. Add ?evaluation=true to run it.',
      responseTime: 0,
      timestamp,
      remediation: 'The evaluation stage check is opt-in to conserve tokens. Add ?evaluation=true to the gateway-test request to enable.',
    };
  }
  ...
  try {
    ...
    return {
      status: 'ok',
      detail: `Evaluation stage check completed. Answers: ${Object.keys(answers).join(', ')}`,
      responseTime,
      timestamp,
      outputTokens: outputTokens,
      evaluationValidated: validationResult.isValid,
      confidenceLevel,
