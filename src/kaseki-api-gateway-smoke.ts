...
export interface ClassificationSmokeTestResult {
  ...
  confidenceLevel?: 'high' | 'medium' | 'low'; // Confidence level of classification results
  confidenceDetails?: { failedQuestions?: string[]; messages?: string[] }; // Details of failed confidence validations
  remediation?: string; // Error recovery steps
...
/**
 * @deprecated No longer used; classification smoke test now uses OpenRouter instead of Cloudflare
 * Kept for backward compatibility but should be removed in a future version.
 */
...
function resolveClassificationConfig(timestamp: string): ClassificationConfig {
  const apiKeyResult = resolveOpenRouterApiKey();
  ...
  if (!apiKeyResult.configured) {
    return {
      ...
      result: {
        ...
        responseTime: 0,
        timestamp,
        remediation: 'Set OPENROUTER_API_KEY or OPENROUTER_API_KEY_FILE environment variable before running the classification smoke test.',
      },
    };
  }
  ...
  return {
    ok: true,
    accountId: '', // Not used for OpenRouter, kept for backward compat
    apiToken: apiKeyResult.value || '', // Use as bearer token for OpenRouter
    model,
  };
}
...
function buildClassificationSmokeRequest(): Record<string, unknown> {
  return {
    ...
    questions: {
      code_quality_issue: {
        type: 'noul',
        instructions: 'Is there a clear code quality issue?',
      },
      requires_human_review: {
        type: 'choice',
        instructions: 'Does this code require human review?',
        criteria: {
          yes: 'Code needs review before merge',
          no: 'Code is safe to merge',
          maybe: 'Unclear - needs clarification',
        },
      },
      fix_risk_level: {
        type: 'score',
        instructions: 'What is the risk level of this fix?',
...
export async function testClassificationSmoke(requested: boolean = false): Promise<ClassificationSmokeTestResult> {
  ...
  try {
    ...
    if (!validationResult.isValid) {
      // If there are failed questions, determine if it's medium or low confidence
      const failedCount = validationResult.failedQuestions.length;
      const totalCount = Object.keys(answers).length;
      const failureRate = totalCount > 0 ? failedCount / totalCount : 0;
      confidenceLevel = failureRate > 0.5 ? 'low' : 'medium';
    }
    ...
    return {
      ...
      confidenceLevel,
      confidenceDetails: validationResult.isValid
        ? undefined
        : {
          failedQuestions: validationResult.failedQuestions,
