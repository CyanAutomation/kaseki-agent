...
/**
 * Classification Smoke Test Result: Native JEV (Cloudflare AI Run)
 * Tests the Cloudflare AI Run endpoint with a native structured classification payload
 * CONSUMES inference tokens - only run when explicitly triggered
 */
export interface ClassificationSmokeTestResult {
  status: 'ok' | 'error' | 'skipped';
  detail: string;
  responseTime: number; // milliseconds
  timestamp: string; // ISO 8601
  modelUsed?: string; // e.g., 'typesafe/jev'
  outputTokens?: number; // Tokens generated by classifier
  classificationValidated?: boolean; // Classification payload was properly formed and responded
  remediation?: string; // Error recovery steps
  httpStatus?: number; // HTTP status from Cloudflare endpoint
  diagnostics?: {
    fieldsSearched?: string[];
    fieldsFound?: string[];
...
/**
 * Resolve Cloudflare AI Run credentials from environment
 */
export type ClassificationConfig =
  | { ok: true; accountId: string; apiToken: string; model: string }
  | { ok: false; result: ClassificationSmokeTestResult };

function resolveCloudflareApiToken(): string {
  if (process.env.CLOUDFLARE_API_TOKEN) {
    return process.env.CLOUDFLARE_API_TOKEN;
  }

  const secretsDir = process.env.KASEKI_SECRETS_DIR || '~/.kaseki';
  const expandedPath = secretsDir.startsWith('~')
    ? secretsDir.replace('~', process.env.HOME || '')
    : secretsDir;
  const secretPath = path.join(expandedPath, 'cloudflare_api_token');

  try {
    const token = fs.readFileSync(secretPath, 'utf8').trim();
    if (token) return token;
  } catch {
    // Continue to return empty string below
  }
...
function resolveClassificationModel(): string {
  return process.env.KASEKI_CLASSIFICATION_MODEL || 'typesafe/jev';
}
...
function resolveClassificationConfig(timestamp: string): ClassificationConfig {
  ...
  const model = resolveClassificationModel();
  ...
  if (!accountId) {
    return {
      ...
      result: {
        ...
        responseTime: 0,
        timestamp,
        remediation: 'Set CLOUDFLARE_ACCOUNT_ID environment variable to your Cloudflare account ID (e.g., abc123def456ghi789)',
      },
    };
  }
  ...
  if (!apiToken) {
    return {
      ...
      result: {
        status: 'error',
        detail: 'CLOUDFLARE_API_TOKEN is not configured',
        responseTime: 0,
        timestamp,
        remediation: 'Set CLOUDFLARE_API_TOKEN or provide a readable cloudflare_api_token file in KASEKI_SECRETS_DIR',
      },
    };
  }

  return { ok: true, accountId, apiToken, model };
}
...
function parseClassificationResult(response: unknown): { result: Record<string, unknown>; outputTokens?: number } | null {
  ...
  return { result: obj.result as Record<string, unknown>, outputTokens };
}

export function shouldRunClassificationSmoke(requested: boolean): boolean {
  // For starter, classification smoke is opt-in via request parameter
  // In future, can add env var override similar to Pi provider smoke
  return requested === true;
}

/**
 * Test Cloudflare AI Run classification model (native JEV payload)
...
export function testClassificationSmoke(requested: boolean = false): ClassificationSmokeTestResult {
  ...
  if (!shouldRunClassificationSmoke(requested)) {
    return {
      status: 'skipped',
      detail: 'Classification smoke skipped. Add ?classification=true to run it.',
      responseTime: 0,
      timestamp,
      remediation: 'Classification smoke is opt-in to conserve tokens. Add ?classification=true to the gateway-test request to enable.',
    };
  }

  const config = resolveClassificationConfig(timestamp);
  if (!config.ok) {
    return config.result;
  }

  // Build the Cloudflare AI Run endpoint URL (will be used in Phase 2 for actual fetch)
  const _endpointUrl = `https://api.cloudflare.com/client/v4/accounts/${config.accountId}/ai/run`;
  const requestBody = buildClassificationSmokeRequest();

  // Request will be sent via fetchWithTimeout in Phase 2 implementation
  void (_endpointUrl); // Mark as intentionally unused for now
  void (requestBody); // Placeholder for Phase 2
  void (parseClassificationResult); // Placeholder for Phase 2

  try {
    // Note: In actual implementation, this would use fetchWithTimeout
    // For now, we return a structured error that matches the test expectations
    return {
      status: 'error',
      detail: 'Classification smoke test: implementation in progress',
      responseTime: Math.round(performance.now() - startTime),
      timestamp,
      modelUsed: config.model,
      remediation: 'Classification smoke feature is under development',
    };
  } catch (error) {
    const responseTime = Math.round(performance.now() - startTime);
    return {
      status: 'error',
      detail: `Classification endpoint error: ${error instanceof Error ? error.message : String(error)}`,
      responseTime,
      timestamp,
      modelUsed: config.model,
      remediation: 'Check Cloudflare account ID, API token, and gateway health before retrying.',
    };
  }
}
