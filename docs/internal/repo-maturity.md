...
- [DevOps/Container Tool Weights](#️-phase-2-devopscontainer-tool-weights) — Custom weights for this project type
...
- [Production Readiness Roadmap](#️-phase-5-production-readiness-roadmap) — Path to 95+, industry best practices
...
| Safe/mock/demo mode | Explicit `--dry-run`, `--test`, or demo mode | `--doctor` is health-check only; no explicit dry-run |
...
| Dependency automation setup | Dependabot, Renovate, or similar | Not visible in current repo; manual updates likely |
...
| Signal | Detection | Evidence for Kaseki |
|--------|-----------|-------------------|
| Issue templates exist | .github/ISSUE_TEMPLATE/* files | Not visible in current structure |
| PR templates exist | .github/PULL_REQUEST_TEMPLATE/* files | Not visible in current structure |
| Labels configured | ≥3 GitHub labels defined | Not visible in current structure |
...
## Phase 2: DevOps/Container Tool Weights
...
| Signal | Detection | Kaseki Status |
|--------|-----------|---------------|
| Image scanning configured | Trivy, Snyk, or GitHub container scanning in CI | NO: not currently in GitHub Actions |
...
| Signal | Detection | Kaseki Status |
|--------|-----------|---------------|
| Container runs as non-root | USER UID:GID in Dockerfile (not UID 0) | YES: USER 10000:10000 |
...
| Signal | Detection | Kaseki Status |
|--------|-----------|---------------|
| Images signed (cosign) OR provenance tracked | Signed image metadata OR SBOM | NO: not currently |
...
| Cache hit rate metrics available | Logs show "cache hit" vs "cache miss" | PARTIAL: happens in logs but no metrics dashboard |
...
| Signal | Detection | Kaseki Status |
|--------|-----------|---------------|
| Build time benchmarks tracked | Docker build duration logged; baseline known | PARTIAL: validation-timings.tsv exists |
...
| Maturity score tracked over time | JSON report artifact; score trend visible | NO: Score not tracked in CI artifacts |
...
1. **Governance & Community (4/10)** — Missing issue/PR templates, labels, CODEOWNERS
   - Fix effort: <1 hour
   - Score gain: +1–2
   - Impact: High (signals professionalism & community readiness)

2. **Runtime Operability (11.2/14)** — No explicit `--dry-run` / demo mode
   - Fix effort: 1–2 hours
   - Score gain: +1
   - Impact: Medium (useful for safe testing)

3. **Security & Hygiene (12.8/16)** — No Dependabot or image scanning in CI
