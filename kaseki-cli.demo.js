...
// Demo commands
demo('1. List all instances', 'NODE_PATH=/workspaces/kaseki-agent node -e "const kasekiCli = require(\\\"./kaseki-cli-lib.js\\\"); kasekiCli.config.KASEKI_RESULTS_DIR = \\\"/tmp/kaseki-cli-demo/agents/kaseki-results\\\"; const instances = kasekiCli.listInstances(); console.log(JSON.stringify(instances, null, 2));"');

demo('2. Get status of a running instance', 'NODE_PATH=/workspaces/kaseki-agent node -e "const kasekiCli = require(\\\"./kaseki-cli-lib.js\\\"); kasekiCli.config.KASEKI_RESULTS_DIR = \\\"/tmp/kaseki-cli-demo/agents/kaseki-results\\\"; const status = kasekiCli.getInstanceStatus(\\\"kaseki-2\\\"); console.log(JSON.stringify(status, null, 2));"');

demo('3. Detect errors in failed instance', 'NODE_PATH=/workspaces/kaseki-agent node -e "const kasekiCli = require(\\\"./kaseki-cli-lib.js\\\"); kasekiCli.config.KASEKI_RESULTS_DIR = \\\"/tmp/kaseki-cli-demo/agents/kaseki-results\\\"; const errors = kasekiCli.detectErrors(\\\"kaseki-3\\\"); console.log(JSON.stringify({instance: \\\"kaseki-3\\\", errorCount: errors.length, errors}, null, 2));"');

demo('4. Get post-run analysis', 'NODE_PATH=/workspaces/kaseki-agent node -e "const kasekiCli = require(\\\"./kaseki-cli-lib.js\\\"); kasekiCli.config.KASEKI_RESULTS_DIR = \\\"/tmp/kaseki-cli-demo/agents/kaseki-results\\\"; const analysis = kasekiCli.getAnalysis(\\\"kaseki-1\\\"); console.log(JSON.stringify(analysis, null, 2));"');
