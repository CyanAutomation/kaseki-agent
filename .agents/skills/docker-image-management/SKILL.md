...
RUN addgroup -g 10000 kaseki && \
    adduser -D -u 10000 -G kaseki kaseki
...
**Test: Non-root user**
```bash
docker run --rm kaseki-template:latest id -u | grep -q 10000
```
...
echo "✓ Non-root user (UID 10000)"
docker run --rm $IMAGE id -u | grep -q 10000
