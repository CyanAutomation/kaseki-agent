...
KASEKI_ARTIFACT_CONSOLIDATION_HELPER="${KASEKI_ARTIFACT_CONSOLIDATION_HELPER:-${KASEKI_SCRIPT_DIR}/scripts/lib/artifact-consolidation.sh}"
if [ ! -r "$KASEKI_ARTIFACT_CONSOLIDATION_HELPER" ] && [ -r /app/scripts/lib/artifact-consolidation.sh ]; then
  KASEKI_ARTIFACT_CONSOLIDATION_HELPER="/app/scripts/lib/artifact-consolidation.sh"
fi
if [ ! -r "$KASEKI_ARTIFACT_CONSOLIDATION_HELPER" ]; then
  printf 'ERROR: Artifact consolidation helper is not readable. Expected %s or /app/scripts/lib/artifact-consolidation.sh.\n' "$KASEKI_ARTIFACT_CONSOLIDATION_HELPER" >&2
  exit 66
fi
# shellcheck source=/dev/null
. "$KASEKI_ARTIFACT_CONSOLIDATION_HELPER"
source_status=$?
if [ "$source_status" -ne 0 ]; then
  printf 'ERROR: Failed to source %s (exit code: %d)\n' "$KASEKI_ARTIFACT_CONSOLIDATION_HELPER" "$source_status" >&2
  exit 1
fi

KASEKI_PI_JSON_CAPTURE_HELPER="${KASEKI_PI_JSON_CAPTURE_HELPER:-${KASEKI_SCRIPT_DIR}/scripts/lib/pi-json-capture.sh}"
if [ ! -r "$KASEKI_PI_JSON_CAPTURE_HELPER" ] && [ -r /app/scripts/lib/pi-json-capture.sh ]; then
  KASEKI_PI_JSON_CAPTURE_HELPER="/app/scripts/lib/pi-json-capture.sh"
fi
if [ ! -r "$KASEKI_PI_JSON_CAPTURE_HELPER" ]; then
  printf 'ERROR: Pi JSON capture helper is not readable. Expected %s or /app/scripts/lib/pi-json-capture.sh.\n' "$KASEKI_PI_JSON_CAPTURE_HELPER" >&2
  exit 66
fi
# shellcheck source=/dev/null
. "$KASEKI_PI_JSON_CAPTURE_HELPER"
source_status=$?
if [ "$source_status" -ne 0 ]; then
  printf 'ERROR: Failed to source %s (exit code: %d)\n' "$KASEKI_PI_JSON_CAPTURE_HELPER" "$source_status" >&2
  exit 1
fi
...
KASEKI_EVALUATION_PROMPTS_HELPER="${KASEKI_EVALUATION_PROMPTS_HELPER:-${KASEKI_SCRIPT_DIR}/scripts/evaluation-prompts.sh}"
if [ ! -r "$KASEKI_EVALUATION_PROMPTS_HELPER" ] && [ -r /app/scripts/evaluation-prompts.sh ]; then
  KASEKI_EVALUATION_PROMPTS_HELPER="/app/scripts/evaluation-prompts.sh"
fi
if [ ! -r "$KASEKI_EVALUATION_PROMPTS_HELPER" ]; then
  printf 'ERROR: Evaluation prompts helper is not readable. Expected %s or /app/scripts/evaluation-prompts.sh.\n' "$KASEKI_EVALUATION_PROMPTS_HELPER" >&2
  exit 66
fi
# shellcheck source=/dev/null
. "$KASEKI_EVALUATION_PROMPTS_HELPER"
source_status=$?
if [ "$source_status" -ne 0 ]; then
  printf 'ERROR: Failed to source %s (exit code: %d)\n' "$KASEKI_EVALUATION_PROMPTS_HELPER" "$source_status" >&2
  exit 1
fi
...
KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER="${KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER:-${KASEKI_SCRIPT_DIR}/scripts/restore-disallowed-changes.sh}"
if [ ! -r "$KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER" ] && [ -r /app/scripts/restore-disallowed-changes.sh ]; then
  KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER="/app/scripts/restore-disallowed-changes.sh"
fi
if [ ! -r "$KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER" ] && [ -r /usr/local/bin/scripts/restore-disallowed-changes.sh ]; then
  KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER="/usr/local/bin/scripts/restore-disallowed-changes.sh"
fi
if [ ! -r "$KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER" ]; then
  printf 'ERROR: Restore-disallowed-changes helper is not readable. Expected %s, /app/scripts/restore-disallowed-changes.sh, or /usr/local/bin/scripts/restore-disallowed-changes.sh. This worker image or mounted template is incomplete; rebuild the image or restore scripts/restore-disallowed-changes.sh.\n' "$KASEKI_RESTORE_DISALLOWED_CHANGES_HELPER" >&2
  exit 66
fi
...
run_pi_event_filter_export() {
  ...
  if [ "$filter_exit" -eq 0 ]; then
    ...
    consolidate_completed_phase "${KASEKI_RESULTS_DIR}"/all-phase-summaries.json "pi-agent" "$summary_file"
    return 0
  fi
...
build_scouting_prompt() {
  ...
  if [ "$use_compact_guidance" -eq 1 ]; then
    cat "$SCRIPT_DIR/templates/scouting/compact.txt"
  else
    # Build base prompt (always included)
    cat "$SCRIPT_DIR/templates/scouting/base.txt"
  fi

  # Conditionally include detailed guidance for complex tasks
  if [ $use_detailed_guidance -eq 1 ]; then
    cat "$SCRIPT_DIR/templates/scouting/detailed-test-impact.txt"
  elif [ "$use_compact_guidance" -eq 0 ]; then
    # For simple tasks, include minimal guidance
    cat "$SCRIPT_DIR/templates/scouting/minimal-test-impact.txt"
  fi
  
  # Common section included for all complexity levels
  if [ "$use_compact_guidance" -eq 0 ]; then
    cat "$SCRIPT_DIR/templates/scouting/common.txt"
    printf '\n## [ORIGINAL TASK PROMPT FOR REFERENCE]\n\nSee goal-setting artifact (%s) if available for upgraded goal with SMART criteria, anti-patterns, and constraints. Otherwise, use the original task prompt below.\n\nOriginal task (before goal-setting upgrade):\n%s\n' \
      "$GOAL_SETTING_ARTIFACT" "$TASK_PROMPT"
  else
    cat <<EOF
...
run_scouting_agent() {
  ...
  consolidate_completed_phase "${KASEKI_RESULTS_DIR}"/all-phase-summaries.json "scouting" "${KASEKI_RESULTS_DIR}"/scouting-summary.json
...
NODE
      ...
      # For patch runs, retain the failure evidence but use the conservative,
...
' "$GOAL_CHECK_CANDIDATE_ARTIFACT" "$GOAL_CHECK_RAW_EVENTS" "${KASEKI_RESULTS_DIR}"/goal-check-events.jsonl "$attempt" 2>"${KASEKI_RESULTS_DIR}/goal-check-recovery-stderr.log"
    if [ -s "${KASEKI_RESULTS_DIR}/goal-check-recovery-stderr.log" ]; then
      echo "[goal-check] assistant-text artifact recovery failed; see goal-check-recovery-stderr.log" >&2
      cat "${KASEKI_RESULTS_DIR}/goal-check-recovery-stderr.log" >&2
    else
      rm -f "${KASEKI_RESULTS_DIR}/goal-check-recovery-stderr.log"
    fi
  fi
...
Recovery attempt: the previous response did not satisfy the run-evaluation schema. Preserve the supplied evidence context. Before finishing, return exactly one valid JSON object with every required field. Do not write files, use markdown/code fences, or return prose. Kaseki validates and persists the response."
  ...
  consolidate_completed_phase "${KASEKI_RESULTS_DIR}"/all-phase-summaries.json "run-evaluation" "${KASEKI_RESULTS_DIR}"/run-evaluation-summary.json
...
KASEKI_GITHUB_PREFLIGHT_AUTH_HELPER="${KASEKI_GITHUB_PREFLIGHT_AUTH_HELPER:-${KASEKI_SCRIPT_DIR}/scripts/github-preflight-auth.sh}"
if [ ! -r "$KASEKI_GITHUB_PREFLIGHT_AUTH_HELPER" ] && [ -r /app/scripts/github-preflight-auth.sh ]; then
  KASEKI_GITHUB_PREFLIGHT_AUTH_HELPER="/app/scripts/github-preflight-auth.sh"
fi
if [ ! -r "$KASEKI_GITHUB_PREFLIGHT_AUTH_HELPER" ]; then
  printf 'ERROR: GitHub preflight auth helper is not readable. Expected %s or /app/scripts/github-preflight-auth.sh. This worker image or mounted template is incomplete; rebuild the image or restore scripts/github-preflight-auth.sh.
